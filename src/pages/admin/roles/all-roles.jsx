...
export default (props) => {
    ...
    const deleteRole = (roleId) => {
        dispatch(deleteRoleAsync({id: roleId}))
    }
    ...
    return (
        ...
        onPtrRefresh={(done) => { 
            ...
            <Navbar backLink="Back" sliding title="Roles">
                <NavRight>
                    {roles.length != 0 && 
                    <Link
            ...
            {!error &&
            <Fab position="right-bottom" slot="fixed" text="Create" color="green" href="/new-role/" />
            }
            ...
            <PageContent className="display-flex justify-content-center text-align-center">
                <div>
                    <h3>Hmm...There are no roles listed yet.</h3>
            ...
            <List mediaList className="search-list searchbar-found">
                {roles.map((role)=>
                    ...
                    key={role.id}
                    title={role.name}
                    subtitle={`${role.users.length} User(s)`}
                    link={`/role/${role.id}`}
                    >
                        <SwipeoutActions left>
                            <SwipeoutButton
                            onClick={()=>{ f7router.navigate(`/edit-role/${role.id}`)}}
                            overswipe
                            color="blue"
                            text="Edit"
                            >
